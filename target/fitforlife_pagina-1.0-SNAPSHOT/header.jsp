...
<div class="relative flex min-h-screen">
    ...
    <aside id="sidebar" class="bg-white shadow-md w-64 flex-shrink-0 absolute inset-y-0 left-0 transform -translate-x-full md:relative md:translate-x-0 transition-transform duration-300 ease-in-out z-30">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <header class="flex justify-between items-center p-4 bg-white border-b z-10">
            ...
            <div class="flex-grow"></div> <div class="flex items-center">
                ...
                <c:choose>
                    <c:when test="${not empty sessionScope.usuario.fotoPerfil}">
                        <img class="h-10 w-10 rounded-full object-cover" src="imagenes/${sessionScope.usuario.fotoPerfil}" alt="Foto de perfil">
                    </c:when>
                    <c:otherwise>
                        <div class="h-10 w-10 rounded-full bg-green-500 flex items-center justify-center text-white font-bold text-xl">
                            ${sessionScope.usuario.nombre.substring(0,1)}
                        </div>
                    </c:otherwise>
                </c:choose>
                <a href="LogoutServlet" class="ml-4 text-sm text-gray-600 hover:text-red-600 font-semibold">Cerrar Sesión</a>
            </div>
        </header>
